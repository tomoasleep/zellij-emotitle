...
describe("when the target is pane", () => {
  describe("when the target is specified by pane_id", () => {
    describe("when the emoji type is pinned", () => {
      test.todo("should apply emojis to the pane specified by pane_id");
    });
  ...
  describe("when the target is not specified", () => {
    describe("when the emoji type is pinned", () => {
      ...
      test("should keep only pinned segments when mixed emojis are set", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
      ...
      test("should keep all pinned segments and remove non-pinned on focus regain", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
      ...
      test("should keep other pane emojis when pane is deleted", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
    ...
    describe("when the emoji type is non-pinned", () => {
      test("should remove emojis on focus change", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
      ...
      test("should remove emojis after 1 second on focused pane", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
      ...
      test("should not resurrect deleted emojis when new emojis are set", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
...
describe("when the target is tab", () => {
  describe("when the target is specified by pane_id", () => {
    describe("when the emoji type is pinned", () => {
    ...
    describe("when the emoji type is non-pinned", () => {
      test.todo("should apply emojis to the tab containing the pane and remove after tab switch");

      test("should keep tab names aligned when tab index changes due to deletion", async () => {
        using zellijSession = await launchZellijSession();
        const { session, configDir, cacheDir, sessionName } = zellijSession;
  ...
  describe("when the target is specified by tab_index", () => {
    describe("when the emoji type is pinned", () => {
      test.todo("should apply emojis to the tab specified by tab_index");
    });
